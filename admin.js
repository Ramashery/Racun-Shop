...
function renderCategories() {
    ...
    ['prodcatsel', 'pf-cat'].forEach(id => {
        ...
        sel.innerHTML = pre + state.categories.map(c => {
            // value = categoryId (сохраняется в БД), label = slug категории (для понимания URL)
            const catSlug = c.slug || c.globalFields?.slug || c.id;
            return `<option value="${c.id}"${c.id === prev ? ' selected' : ''}>${escH(catSlug)}</option>`;
        }).join('');
    });
}
...
window.updProdUrl = function() {
    ...
    // URL товара строится из slug категории (поле slug документа категории),
    // а не из categoryId — поэтому находим объект категории и берём slug из него.
    const catId   = document.getElementById('prodcatsel')?.value || '';
    const catObj  = state.categories.find(c => c.id === catId);
    const catSlug = catObj?.slug || catObj?.globalFields?.slug || catId || 'category-slug';
    setText('produp', `/en/products/${catSlug}/${slug}/`);
};
...
window.updCatUrl = function() {
    // URL категории строится из поля slug документа категории (не из id).
    if (currentCollection === 'categories' && currentEditingId) {
        const catObj  = state.categories.find(c => c.id === currentEditingId);
        // Берём slug из поля формы (актуальное значение) или из объекта категории
        const catSlug = getV('catslug') || catObj?.slug || catObj?.globalFields?.slug || currentEditingId;
        setText('caturlprev', `/en/products/${catSlug}/`);
    } else {
        // При создании новой категории slug задаётся в поле формы
        const draftSlug = getV('catslug');
        setText('caturlprev', draftSlug ? `/en/products/${draftSlug}/` : `/en/products/{slug}/`);
    }
};
