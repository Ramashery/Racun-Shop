...
window.saveCategory = async function() {
    ...
    // При создании новой категории ID задаётся явно из поля catIdInput,
    // чтобы document ID = URL-сегмент = categoryId у товаров этой категории.
    const catDocId = currentEditingId ? null : getV('catIdInput').trim();
    if (!currentEditingId && !catDocId) {
        showToast('Please fill in the Category ID field', 'err'); return;
    }
