...
window.updCatUrl = function() {
    // Реальная страница категории живёт на /products/{categoryId} — это id документа
    // категории (то же значение, что и categoryId у товаров внутри этой категории),
    // а НЕ поле "slug" из этой формы. Поле slug сейчас не участвует в адресе страницы.
    if (currentCollection === 'categories' && currentEditingId) {
        setText('caturlprev', `/en/products/${currentEditingId}/`);
    } else {
        setText('caturlprev', `/en/products/{ID — присвоится при создании}/`);
    }
};
