...
window._editProd = function(id) {
    ...
    LANGS.forEach(lang => {
        ...
        renderCtaBtns(lang, p.translations?.[lang]?.ctaButtons || []);
    });
...
window.saveProduct = async function() {
    ...
    LANGS.forEach(lang => {
        ...
        obj.ctaButtons        = collectCtaBtns(lang);
...
window.savePageDoc = async function(pageId) {
    ...
    if (pageId === 'home') {
        ...
        globalFields.regionHreflangMap = { 'en': getV('home-regionHreflang-en'), 'ru': getV('home-regionHreflang-ru'), 'ka': getV('home-regionHreflang-ka'), 'hy': getV('home-regionHreflang-hy') };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (IS_CONTENT) {
        ...
        document.getElementById('btn-new-prod')?.addEventListener('click', () => {
            ...
            LANGS.forEach(lang => {
                const b = document.getElementById(`${lang}-descpoints-body`); if (b) b.innerHTML = '';
                const c = document.getElementById(`${lang}-cta-body`); if (c) c.innerHTML = '';
            });
            updProdUrl(); showM('mprod');
        });
...
// ════════════════════════════════════════════════════════
// CTA BUTTONS — dynamic list per language
// Stored as: translations[lang].ctaButtons = [{label, url}, …]
// ════════════════════════════════════════════════════════
function renderCtaBtns(lang, btns) {
    const body = document.getElementById(`${lang}-cta-body`);
    if (!body) return;
    if (!btns || !btns.length) { body.innerHTML = ''; return; }
    body.innerHTML = btns.map((b, i) => `
        <div class="cta-row" style="display:flex;align-items:flex-start;gap:8px;margin-bottom:8px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;padding:10px 12px">
          <div style="flex:1;min-width:0">
            <div style="font-size:11px;font-weight:700;color:var(--text2);text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px">Button ${i + 1}</div>
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              <input class="fin cta-label" style="flex:1;min-width:120px" placeholder="Button label (e.g. Write on Telegram)" value="${escH(b.label || '')}">
              <input class="fin cta-url"   style="flex:2;min-width:180px" placeholder="https://t.me/… or mailto:… or https://wa.me/…" value="${escH(b.url || '')}">
            </div>
          </div>
          <button class="ib dr" style="flex-shrink:0;margin-top:22px" onclick="removeCtaBtn('${lang}',${i})" title="Remove"><span class="material-symbols-outlined ic">delete</span></button>
        </div>`).join('');
}

window.addCtaBtn = function(lang) {
    const btns = collectCtaBtns(lang);
    btns.push({ label: '', url: '' });
    renderCtaBtns(lang, btns);
    const body = document.getElementById(`${lang}-cta-body`);
    if (body) {
        const rows = body.querySelectorAll('.cta-row');
        const last = rows[rows.length - 1];
        if (last) last.querySelector('.cta-label')?.focus();
    }
};

window.removeCtaBtn = function(lang, idx) {
    const btns = collectCtaBtns(lang);
    btns.splice(idx, 1);
    renderCtaBtns(lang, btns);
};

function collectCtaBtns(lang) {
    const body = document.getElementById(`${lang}-cta-body`);
    if (!body) return [];
    return Array.from(body.querySelectorAll('.cta-row')).map(r => ({
        label: r.querySelector('.cta-label')?.value.trim() || '',
        url:   r.querySelector('.cta-url')?.value.trim()   || '',
