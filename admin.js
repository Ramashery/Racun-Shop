...
// Зеркало slugify() из generate_site.py — используется, чтобы ID нового документа
// категории совпадал с тем, что реально попадёт в URL (а не было случайной строкой,
// которую генерирует Firestore через addDoc()).
const CYR_MAP = {
...
function slugifyId(text) {
    ...
    s = s.split('').map(c => CYR_MAP[c] ?? c).join('');
...
window.saveCategory = async function() {
    ...
    if (!slug) { showToast('Please enter a slug', 'err'); return; }
    ...
    if (currentEditingId) {
        // Редактирование существующей категории — ID документа не меняем.
        await persistToFirestore('categories', baseData);
        return;
    }

    // Новая категория: ID документа = слаг (а не случайная строка от addDoc()),
    // потому что именно ID документа = categoryId, который попадёт и в URL
    // страницы категории, и в URL товаров внутри неё.
    const catId = slugifyId(slug);
    if (state.categories.some(c => c.id === catId)) {
        showToast(`Категория с ID "${catId}" уже существует — выберите другой слаг`, 'err');
        return;
    }

    const data = { ...baseData, id: catId };
    try {
        await setDoc(doc(db, 'categories', catId), data);
        state.categories.push({ id: catId, ...data });
        showToast('✅ Created in Firebase!');
        renderAll();
        closeM('mcat');
        currentEditingId = null;
...
window.updProdUrl = function() {
    ...
    // Реальный URL строится из categoryId (значения опции), а не из её подписи —
    // показываем то же самое, что действительно попадёт в адрес страницы.
...
window.updCatUrl = function() {
    // ID документа категории = слаг (slugifyId), и именно он становится URL
    // страницы категории и categoryId товаров внутри неё.
    if (currentCollection === 'categories' && currentEditingId) {
        // Редактируем существующую категорию — её ID документа уже зафиксирован
        // и не меняется, даже если сейчас в поле slug введено другое значение.
        setText('caturlprev', `/en/products/${currentEditingId}/`);
    } else {
        const raw = getV('catslug');
        setText('caturlprev', raw ? `/en/products/${slugifyId(raw)}/` : '/en/products/{слаг}/');
    }
};
