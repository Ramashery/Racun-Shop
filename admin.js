// ============================================================
//  RAKUN Admin Panel — Firebase SPA Controller (v4.0)
...
// IndexedDB: данные из кэша приходят мгновенно при повторном открытии
...
// FIX #1: правильное чтение h1 поста из translations.{lang}.h1
...
// FIX #1: правильное чтение картинки поста
...
// ════════════════════════════════════════════════════════
// 1. ЗАГРУЗКА — onSnapshot (FIX #4: данные сразу из кэша)
// ════════════════════════════════════════════════════════
...
function renderCategories() {
    ...
    // FIX #3: мобильные карточки категорий
...
function renderProducts() {
    ...
    // FIX #3: мобильные карточки продуктов
...
function renderBlog() {
    ...
    const list = state.posts.filter(p => {
        ...
        const title  = getPostTitle(p);   // FIX #1
        const img    = getPostImage(p);   // FIX #1
    ...
    if (tb) tb.innerHTML = list.map(({ p, title, img, cat, author, status, slug }) => `<tr>
      <td><div class="pnc">
        <img src="${escH(img)}" class="pth" style="object-fit:cover" onerror="this.onerror=null;this.style.cssText='background:#2d3148;width:44px;height:44px;border-radius:6px;flex-shrink:0';this.removeAttribute('src')">
    ...
    // FIX #3: мобильные карточки блога
    if (mob) mob.innerHTML = list.map(({ p, title, img, cat, status }) =>
        `<div class="mc">
          <div class="mct">
            <div class="pnc" style="gap:10px">
...
// ════════════════════════════════════════════════════════
// 6. НАВИГАЦИЯ (FIX #2: sessionStorage сохраняет раздел)
// ════════════════════════════════════════════════════════
...
window.goP = function(id) {
    ...
    closeMob();
    // FIX #2: сохраняем в sessionStorage
    try { sessionStorage.setItem(SESSION_KEY, id); } catch(e) {}
};

// FIX #2: восстановление раздела при загрузке
function restoreSection() {
    ...
    if (IS_CONTENT) goP('products');
    if (IS_PAGES)   goP('page-home');
}
...
window.openMobMenu  = () => { document.getElementById('mob-menu')?.classList.add('on'); document.getElementById('mob-menu-overlay')?.classList.add('on'); document.body.style.overflow = 'hidden'; };
window.closeMobMenu = () => { document.getElementById('mob-menu')?.classList.remove('on'); document.getElementById('mob-menu-overlay')?.classList.remove('on'); document.body.style.overflow = ''; };
...
window.addProdVolume = function() {
    const vols = collectProdVolumes(); vols.push({label:'',value:'',link:''}); renderProdVolumes(vols);
};
...
window.addProdDosage = function() {
    const rows = collectProdDosage(); rows.push({type:'',weight:'',amount:''}); renderProdDosage(rows);
};
...
// ════════════════════════════════════════════════════════
// 8. PAGES & SETTINGS (admin-pages.html)
// ════════════════════════════════════════════════════════
...
function fillHomePage(lang) {
    ...
    renderSlides(rawSlides.map(s => ({
        subtitle: s.subtitle || '', headline: s.headline || '',
...
function fillPageFields(pageId, lang) {
    ...
    if (pageId === 'catalog') {
        ...
        // catalog-description — поле упразднено, heroSubtitle является каноническим
...
window.savePageDoc = async function(pageId) {
    ...
    if (pageId === 'home') {
        ...
        localizedFields = {
            label:           getV('about-label'),          title:           getV('about-title'),
            subtitle:        getV('about-subtitle'),        mainHtml:        getV('about-mainHtml'),
            valuesHtml:      getV('about-valuesHtml'),     heroImageAlt:    getV('about-heroImageAlt'),
        ...
        globalFields.foundedYear  = getV('about-foundedYear'); globalFields.headquarters = getV('about-headquarters');
        globalFields.heroImage    = getV('about-heroImage');   globalFields.ogImageUrl   = getV('about-ogImageUrl');
    ...
    const newData = {
        ...
        ...(pageId === 'contacts' ? { stores: collectContactStores() } : {}),
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (IS_CONTENT) {
        ...
        restoreSection(); // FIX #2 — до loadAllData
        loadAllData();    // FIX #4 — onSnapshot
    }
