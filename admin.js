...
//           changeFrequency, xDefaultHreflang, images,
//           featurePills.
//
...
function renderProducts() {
    ...
    const list = state.products.filter(p => {
        ...
        const img     = p.translations?.en?.images?.[0] || p.globalFields?.images?.[0] || '';
...
window._editProd = function(id) {
    ...
    // Images and featurePills: per-lang, but edited as one shared UI block.
    // Read from EN translation first; fall back to globalFields for legacy docs.
    const enT    = p.translations?.en || {};
    const images = Array.isArray(enT.images)       ? enT.images       : (gf.images || []);
    const pills  = Array.isArray(enT.featurePills) ? enT.featurePills : (gf.featurePills || []);

    const imgTa = document.getElementById('prod-images-ta');
    if (imgTa) imgTa.value = images.join('\n');
...
window.saveProduct = async function() {
    ...
    // images and featurePills: collect once from shared UI, write into every lang
    const imgTa        = document.getElementById('prod-images-ta');
    ...
    // Inject images and featurePills into every lang translation object
    LANGS.forEach(lang => { translations[lang].images = images; translations[lang].featurePills = featurePills; });
...
function renderCtaButtons(lang, btns) {
    ...
    body.innerHTML = btns.map((b, i) => `
        <div class="ir cta-btn-row" data-idx="${i}" style="gap:8px;margin-bottom:10px;align-items:center;flex-wrap:wrap">
          <input class="fin cta-btn-label" placeholder="Button label (e.g. Telegram)" value="${escH(b.label || '')}" style="flex:1;min-width:140px">
          <input class="fin cta-btn-url"   placeholder="URL (https://t.me/…)" value="${escH(b.url || '')}" style="flex:2;min-width:200px">
