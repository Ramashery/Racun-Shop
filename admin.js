// ============================================================
//  RAKUN Admin Panel — Unified Firebase Controller (v3.0)
//  Единый модуль логики для всех страниц админ-панели.
//  Подключается через <script type="module" src="/admin.js">
// ============================================================
import {
    initializeApp
...
const PAGE_IDS = ['home', 'catalog', 'blog-list', 'about', 'contacts', 'footer-nav'];
...
const pagesCache = {};
const pageLangs  = { home: 'en', catalog: 'en', 'blog-list': 'en', about: 'en', contacts: 'en', 'footer-nav': 'en' };
...
function getLocalVal(d, key, lang = 'en') {
    ...
    if (d.translations?.en?.[key]     !== undefined) return d.translations.en[key];
...
// ════════════════════════════════════════════════════════════
// 1. ЗАГРУЗКА ДАННЫХ — CONTENT (категории, продукты, посты)
// ════════════════════════════════════════════════════════════
async function loadAllData() {
    ...
    try {
        ...
        console.log('Firebase content loaded:', state);
    } catch (err) {
...
// ════════════════════════════════════════════════════════════
// 2. ЗАГРУЗКА ДАННЫХ — PAGES & SETTINGS
// ════════════════════════════════════════════════════════════
async function loadPagesAndSettings() {
    try {
        await Promise.all(PAGE_IDS.map(async id => {
            const snap = await getDoc(doc(db, 'pages', id));
            pagesCache[id] = snap.exists() ? snap.data() : {};
        }));
        const settSnap = await getDoc(doc(db, 'settings', 'main'));
        pagesCache['settings'] = settSnap.exists() ? settSnap.data() : {};

        // Заполняем все страницы
        PAGE_IDS.forEach(id => fillPageFields(id, pageLangs[id] || 'en'));
        fillSettings();

        setV('fb-status', 'Connected ✓');
        console.log('Pages & settings loaded:', pagesCache);
    } catch (err) {
...
// ════════════════════════════════════════════════════════════
// 3. РЕНДЕРИНГ ТАБЛИЦ (content.html)
// ════════════════════════════════════════════════════════════
function renderAll() {
    renderCategories();
    renderProducts();
...
function renderBlog() {
    ...
    tb.innerHTML = list.map(p => {
        ...
        const status = p.globalFields?.status     || 'Draft';
        const slug   = p.globalFields?.slug       || p.id;
        const img    = p.globalFields?.ogImageUrl || p.globalFields?.images?.[0] || '';
        return `<tr>
          <td><div class="pnc">
...
// ════════════════════════════════════════════════════════════
// 4. ЗАПОЛНЕНИЕ ПОЛЕЙ СТРАНИЦ (pages.html)
// ════════════════════════════════════════════════════════════
function fillPageFields(pageId, lang) {
    ...
    // Универсальный заполнитель SEO-блока
    const fillSEO = (prefix) => {
        setV(`${prefix}-seoTitle`,       lo.seoTitle       || gf.seoTitle       || '');
        setV(`${prefix}-ogTitle`,        lo.ogTitle        || gf.ogTitle        || '');
        setV(`${prefix}-metaDescription`,lo.metaDescription|| gf.metaDescription|| '');
        setV(`${prefix}-ogDescription`,  lo.ogDescription  || gf.ogDescription  || '');
        setV(`${prefix}-ogImageUrl`,     gf.ogImageUrl     || lo.ogImageUrl     || '');
        setV(`${prefix}-schema`,         lo.schema         || gf.schema         || '');
    };

    if (pageId === 'home') {
        setV('home-productsLabel', lo.productsLabel || lo.productsSectionLabel || '');
        setV('home-productsTitle', lo.productsTitle || lo.productsSectionTitle || '');
        setV('home-blogLabel',     lo.blogLabel     || lo.blogSectionLabel     || '');
        setV('home-blogTitle',     lo.blogTitle     || lo.blogSectionTitle     || '');
        setV('home-reviewsLabel',  lo.reviewsLabel  || lo.reviewsSectionLabel  || '');
        setV('home-reviewsTitle',  lo.reviewsTitle  || lo.reviewsSectionTitle  || '');
        fillSEO('home');
        const rawSlides = lo.heroSlides || lo.slides || gf.heroSlides || gf.slides || [];
        renderSlides(rawSlides.map(s => ({
            subtitle:           s.subtitle           || '',
            headline:           s.headline           || '',
            backgroundImageUrl: s.backgroundImageUrl || s.bgImage || '',
            buttonText:         s.buttonText         || s.btnText || '',
            buttonLink:         s.buttonLink         || s.btnUrl  || s.btnLink || '',
        })));
        ...
        setV('catalog-title',       lo.heroTitle       || lo.title       || '');
        setV('catalog-subtitle',    lo.heroSubtitle    || lo.subtitle    || '');
        setV('catalog-description', lo.heroDescription || lo.description || '');
        fillSEO('catalog');

    } else if (pageId === 'blog-list') {
        setV('blog-list-title',    lo.title    || lo.heroTitle    || '');
        setV('blog-list-subtitle', lo.subtitle || lo.heroSubtitle || '');
        fillSEO('blog-list');

    } else if (pageId === 'about') {
        setV('about-heroTitle',   lo.heroTitle   || lo.title       || '');
        setV('about-contentHtml', lo.contentHtml || lo.content     || '');
        setV('about-mainImage',   gf.mainImage   || lo.mainImage   || '');
        fillSEO('about');
        ...
        setV('footer-nav-home',      lo.navHomeText     || lo.home     || '');
        setV('footer-nav-about',     lo.navAboutText    || lo.about    || '');
        setV('footer-nav-catalog',   lo.navCatalogText  || lo.catalog  || '');
        setV('footer-nav-blog',      lo.navBlogText     || lo.blog     || '');
        setV('footer-nav-contacts',  lo.navContactsText || lo.contacts || '');
        setV('footer-nav-copyright', lo.copyright       || gf.copyright|| '');
        setV('footer-nav-link1',     lo.link1           || '');
    }
}

// ════════════════════════════════════════════════════════════
// 5. СОХРАНЕНИЕ СТРАНИЦ (pages.html)
// ════════════════════════════════════════════════════════════
window.switchPageLang = function(pageId, lang) {
    pageLangs[pageId] = lang;
    // Обновляем лейбл кнопки если он есть
...
window.savePageDoc = async function(pageId) {
    ...
    const collectSEO = (prefix) => ({
        seoTitle:        getV(`${prefix}-seoTitle`),
        ogTitle:         getV(`${prefix}-ogTitle`),
        metaDescription: getV(`${prefix}-metaDescription`),
        ogDescription:   getV(`${prefix}-ogDescription`),
        schema:          getV(`${prefix}-schema`),
    });

    let loc    = {};
    let global = { ...existingGF };
    ...
    if (pageId === 'home') {
        ...
        global.ogImageUrl = getV('home-ogImageUrl');
        ...
        global.ogImageUrl = getV('catalog-ogImageUrl');

    } else if (pageId === 'blog-list') {
        loc = {
            ...collectSEO('blog-list'),
            title:    getV('blog-list-title'),
            subtitle: getV('blog-list-subtitle'),
        };
        global.ogImageUrl = getV('blog-list-ogImageUrl');
        ...
        global.mainImage  = getV('about-mainImage');
        global.ogImageUrl = getV('about-ogImageUrl');
        ...
        global = {
            ...global,
            phone:      getV('contacts-phone'),
            email:      getV('contacts-email'),
            mapsEmbed:  getV('contacts-mapsEmbed'),
            ogImageUrl: getV('contacts-ogImageUrl'),
        };
        ...
        loc = {
            ...
            navCatalogText:  getV('footer-nav-catalog'),
            navBlogText:     getV('footer-nav-blog'),
    ...
    const newData = {
        ...
        translations: {
            ...existingTr,
            [lang]: { ...(existingTr[lang] || {}), ...loc },
        },
    };

    try {
        await setDoc(doc(db, 'pages', pageId), newData, { merge: true });
...
// ════════════════════════════════════════════════════════════
// 6. СЛАЙДЫ И ОТЗЫВЫ (home page)
// ════════════════════════════════════════════════════════════
function renderSlides(slides) {
    const body = document.getElementById('home-slides-body');
    if (!body) return;
    body.innerHTML = slides.map((s, i) => `
      <div class="slide-row" data-idx="${i}" style="margin-bottom:18px;padding:16px;background:var(--bg3);border-radius:8px;border:1px solid var(--border2)">
        <div style="font-size:12px;font-weight:700;color:var(--text2);margin-bottom:10px;display:flex;justify-content:space-between;align-items:center">
          SLIDE ${i + 1}
          <button class="ib dr" onclick="removeSlide(${i})"><span class="material-symbols-outlined ic">delete</span></button>
        </div>
        <div class="fg">
          <div class="fi"><label class="fl">Subtitle</label><input class="fin slide-subtitle" value="${escH(s.subtitle)}"></div>
          <div class="fi"><label class="fl">Button Text</label><input class="fin slide-btnText" value="${escH(s.buttonText)}"></div>
          <div class="fi full"><label class="fl">Headline</label><input class="fin slide-headline" value="${escH(s.headline)}"></div>
          <div class="fi full"><label class="fl">Background Image URL</label><input class="fin slide-bgImage" value="${escH(s.backgroundImageUrl)}"></div>
          <div class="fi"><label class="fl">Button URL</label><input class="fin slide-btnUrl" value="${escH(s.buttonLink)}"></div>
        </div>
      </div>`).join('');
}
window.addSlide    = function() { renderSlides([...collectSlides(), { subtitle:'', headline:'', buttonText:'', buttonLink:'', backgroundImageUrl:'' }]); };
...
function renderReviews(reviews) {
    const body = document.getElementById('home-reviews-body');
    if (!body) return;
    body.innerHTML = reviews.map((r, i) => `
      <div class="review-row" style="margin-bottom:14px;padding:14px;background:var(--bg3);border-radius:8px;border:1px solid var(--border2)">
        <div style="font-size:12px;font-weight:700;color:var(--text2);margin-bottom:8px;display:flex;justify-content:space-between;align-items:center">
          REVIEW ${i + 1}
          <button class="ib dr" onclick="removeReview(${i})"><span class="material-symbols-outlined ic">delete</span></button>
        </div>
        <div class="fg">
...
window.addReview    = function() { renderReviews([...collectReviews(), { author:'', role:'', text:'' }]); };
window.removeReview = function(idx) { const rv = collectReviews(); rv.splice(idx, 1); renderReviews(rv); };
...
function fillSettings() {
    ...
    setV('settings-siteName',        s.siteName       || '');
    setV('settings-defaultLang',     s.defaultLang    || 'en');
...
// ════════════════════════════════════════════════════════════
// 8. CRUD — ОТКРЫТИЕ МОДАЛОК (content.html)
// ════════════════════════════════════════════════════════════
...
window._editPost = function(id) {
    ...
    LANGS.forEach(lang => {
        ...
        const map = { h1:0, mediaUrls:1, mainImageAltText:2, cardTitle:3, cardDescription:4, seoTitle:5, metaDescription:6, ogTitle:7, ogDescription:8 };
        Object.entries(map).forEach(([key, idx]) => {
            if (!inputs[idx]) return;
            let val = getLocalVal(p, key, lang);
            if (key === 'mediaUrls' && Array.isArray(val)) val = val.join(', ');
...
// ════════════════════════════════════════════════════════════
// 9. СОХРАНЕНИЕ В FIRESTORE (content.html)
// ════════════════════════════════════════════════════════════
async function persistToFirestore(colName, data) {
    ...
    try {
        if (currentEditingId) {
            ...
            const stateKey = colName === 'blog' ? 'posts' : colName;
            const arr = state[stateKey];
            const i   = arr.findIndex(x => x.id === currentEditingId);
            if (i !== -1) arr[i] = { id: currentEditingId, ...data };
            showToast('✅ Сохранено в Firebase!');
        } else {
            const ref      = await addDoc(collection(db, firestoreCol), data);
            const stateKey = colName === 'blog' ? 'posts' : colName;
            state[stateKey].push({ id: ref.id, ...data });
            showToast('✅ Создано в Firebase!');
        }
        renderAll();
        closeM({ categories: 'mcat', products: 'mprod', posts: 'mblog' }[colName]);
...
window.saveProduct = async function() {
    ...
    await persistToFirestore('products', {
        globalFields: {
            ...
            volume: getV('prodvol'), priority: getV('prodprio'), region: getV('prodregion'),
            ogImageUrl: getV('prodogimg'), images,
        },
        translations,
...
window.saveBlogPost = async function() {
    ...
    LANGS.forEach(lang => {
        ...
        const inputs = pane.querySelectorAll('input.fin');
        const tas    = pane.querySelectorAll('textarea.fta');
        const mediaUrlsArray = (inputs[1]?.value.trim() || '').split(',').map(u => u.trim()).filter(Boolean);
        translations[lang] = {
...
async function confirmDelete() {
    ...
    try {
        await deleteDoc(doc(db, firestoreCol, pendingDeleteId));
        const stateKey   = pendingDeleteCol === 'blog' ? 'posts' : pendingDeleteCol;
        state[stateKey]  = state[stateKey].filter(x => x.id !== pendingDeleteId);
        renderAll();
        closeM('mconf');
        showToast('🗑 Удалено');
    } catch (err) {
...
const PAGE_META = {
    ...
    'page-blog-list':{ t:'Blog Page',            c:'/ Pages' },
...
window.swBlogLang = (lang, btn) => {
    document.querySelectorAll('#mblog .lpane, #mblog .ltab').forEach(p => p.classList.remove('on'));
    document.getElementById(`blog-lp-${lang}`)?.classList.add('on');
    if (btn) btn.classList.add('on');
};
window.swProdLang = (lang, btn) => {
    document.querySelectorAll('#mprod .lpane, #mprod .ltab').forEach(p => p.classList.remove('on'));
...
window.updProdUrl = function() {
    const catObj  = state.categories.find(c => c.id === getV('prodcatsel'));
    const catSlug = catObj?.slug || catObj?.globalFields?.slug || 'category';
...
// ════════════════════════════════════════════════════════════
// 12. ИНИЦИАЛИЗАЦИЯ — определяем страницу и запускаем нужное
// ════════════════════════════════════════════════════════════
document.addEventListener('DOMContentLoaded', () => {
    checkMob();
    window.addEventListener('resize', checkMob);

    // Навигация (клики по .ni[data-p])
    ...
    // ── content.html ──────────────────────────────────────
    const confBtn = document.getElementById('mconf-confirm-btn');
    if (confBtn) confBtn.onclick = confirmDelete;
    ...
    document.getElementById('cat-delete-btn')?.addEventListener('click',  () => { if (currentEditingId) { closeM('mcat');  _askDelete('categories', currentEditingId); } });
    document.getElementById('prod-delete-btn')?.addEventListener('click', () => { if (currentEditingId) { closeM('mprod'); _askDelete('products',   currentEditingId); } });
    document.getElementById('blog-delete-btn')?.addEventListener('click', () => { if (currentEditingId) { closeM('mblog'); _askDelete('posts',      currentEditingId); } });
    ...
    // ── Определяем какой модуль нужен на этой странице ───
    const body = document.body;
    const isContent  = !!document.getElementById('products-tbody');
    const isPages    = !!document.getElementById('pn-page-home');
    const isSettings = !!document.getElementById('settings-siteName');

    if (isContent)  loadAllData();
    if (isPages)    loadPagesAndSettings();
    if (isSettings) loadPagesAndSettings(); // settings.html тоже грузит pagesCache для fillSettings
});
