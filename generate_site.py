...
def build_home_schema(lang, page, contacts, reviews, products):
    ...
    # Берём в список только товары с ценой: без неё нет "offers", а Google
    # Rich Results Test считает Product без offers/review/aggregateRating
    # критической ошибкой и не даёт странице попасть в расширенные результаты.
    listed = [p for p in products if p.get("name") and (p.get("price") or "").strip()][:12]
    if listed:
        item_list = {
            "@type": "ItemList",
