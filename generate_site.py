        ...
        for raw in published:
            ...
            if lang == SUPPORTED_LANGS[0]:
                sitemap_entries.append({
                    ...
                    "regionHreflang": gf(raw).get("regionHreflang") or "",
...
def gen_sitemap(entries):
    ...
    for e in entries:
        slug          = e["slug"]
        entry_type    = e["type"]          # "product" or "post"
        x_default     = e["xDefaultHreflang"]
        region        = e["regionHreflang"]
        langs         = e["urlLang"]
        cat           = e.get("cat", "")   # only for products

        # Build per-language URL helper
        def page_url(l):
            if entry_type == "product":
                return f"{BASE_URL}/{l}/products/{cat}/{slug}/"
            return f"{BASE_URL}/{l}/blog/{slug}/"

        # One <url> block per language variant (Google requirement)
        for lang_var in langs:
            block = ["<url>", f"  <loc>{page_url(lang_var)}</loc>"]

            # xhtml:link for every language
            for l in langs:
                hreflang_val = region if (l == lang_var and region) else l
                block.append(
                    f'  <xhtml:link rel="alternate" hreflang="{hreflang_val}" href="{page_url(l)}"/>'
                )
            # x-default
            block.append(
                f'  <xhtml:link rel="alternate" hreflang="x-default" href="{page_url(x_default)}"/>'
            )
