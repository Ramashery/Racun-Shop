...
def gen_404(data):
    ...
    # Словари UI для страницы 404 (переводы для заголовков и текста)
    UI_STRINGS = {
        "en": {
    ...
    for lang in SUPPORTED_LANGS:
        ...
        # Собираем навигацию и футер точно так же, как на других страницах
        nav = {
            "home":     footer_lo.get("home")     or footer_gf.get("home")     or "Home",
            "about":    footer_lo.get("about")    or footer_gf.get("about")    or "About",
            "catalog":  footer_lo.get("catalog")  or footer_gf.get("catalog")  or "Catalog",
        ...
        # Сохраняем языковую версию 404
        write(f"{OUTPUT_DIR}/{lang}/404.html", html)

        # Важно для Firebase Hosting: корневой файл 404.html по умолчанию 
        # берем из английской версии (так как Firebase ищет его именно в корне /public/404.html)
        if lang == "en":
