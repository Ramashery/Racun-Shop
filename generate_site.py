...
def build_home_schema(lang, page, contacts, reviews, products):
    ...
    # ── адрес: берём первый магазин/склад с реальным (не заглушечным) адресом ──
    address = None
    for s in (contacts.get("stores") or []):
        addr = (s.get("address") or "").strip()
        if addr and addr.lower() not in _PLACEHOLDER_ADDR:
            address = {"@type": "PostalAddress", "streetAddress": addr}
            if s.get("city"):
                address["addressLocality"] = s["city"]
            if s.get("country"):
    ...
    if listed:
        ...
        for i, p in enumerate(listed, start=1):
            ...
            node = {"@type": "Product", "name": p["name"], "url": product_url}
            if p.get("firstImage"):
