...
COPY_IGNORE = {
    ...
    "tpl_about.html", "tpl_category.html",
...
def extract(doc, lang):
    ...
    # _cat_slug используется для построения URL товара: /products/{_cat_slug}/{slug}
    # Для этого ищем slug категории по categoryId товара.
    # Разрешение происходит в gen_products() через _resolve_cat_slug().
    # Здесь сохраняем только сырой categoryId для последующего разрешения.
    cat_id_raw     = g.get("categoryId") or doc.get("categoryId") or "products"
    m["_cat_slug"] = slugify(cat_id_raw)   # временный fallback; перезапишется в gen_products/gen_categories
...
def _build_cat_slug_map(categories):
    """
    Возвращает dict: categoryId (doc.id) -> slug из поля 'slug' документа категории.
    Если поля 'slug' нет — fallback на slugify(id).
    Используется чтобы URL категории и URL товара строились из slug, а не из id.
    """
    mapping = {}
    for c in categories:
        ...
        if not c_id:
            continue
        # slug хранится в корне документа категории (не в globalFields)
        c_slug = c.get("slug") or slugify(c_id)
        mapping[c_id] = c_slug
    return mapping
...
def gen_catalog(data):
    ...
    for lang in SUPPORTED_LANGS:
        ...
        cat_slug_map = _build_cat_slug_map(data["categories"])
        ...
        for cat_raw in data["categories"]:
            ...
            # URL строится из поля slug документа категории, а не из id
            cat_slug = cat_slug_map.get(cat_id) or slugify(cat_id)
            cat_name = cat_lo2.get("name") or cat_id
            if cat_slug:
                categories.append({
                    "slug":        cat_slug,      # slug для URL: /products/{slug}/
                    "urlSlug":     cat_slug,
                    "name":        cat_name,
...
def gen_categories(data):
    ...
    cat_slug_map = _build_cat_slug_map(active_categories)
    ...
    for lang in SUPPORTED_LANGS:
        ...
        for c in active_categories:
            ...
            if not c_id:
                continue
            c_lo = loc(c, lang)
            # Slug для URL берём из поля slug документа категории
            c_slug = cat_slug_map.get(c_id) or slugify(c_id)
        ...
        for c_raw in active_categories:
            ...
            if not c_id:
                continue
            c_lo      = loc(c_raw, lang)
            # URL категории = поле slug документа категории (не slugify от id)
            cat_slug  = cat_slug_map.get(c_id) or slugify(c_id)
            cat_name  = c_lo.get("name") or c_id

            cat_products = [
                extract(p, lang) for p in published
            ]
            # _cat_slug товара тоже разрешаем через маппинг
            cat_products = [
                p for p in cat_products
                if p.get("name", "").strip()
                and (cat_slug_map.get(gf(next((r for r in published if r.get("_id") == p["_id"]), {})).get("categoryId") or "") or "") == cat_slug
            ]
...
def gen_products(data):
    ...
    # Маппинг categoryId -> slug категории для построения правильных URL товаров
    cat_slug_map = _build_cat_slug_map(data["categories"])
    ...
    for lang in SUPPORTED_LANGS:
        ...
        for raw in published:
            ...
            # Перезаписываем _cat_slug через маппинг slug'ов категорий
            raw_cat_id = gf(raw).get("categoryId") or raw.get("categoryId") or "products"
            product["_cat_slug"] = cat_slug_map.get(raw_cat_id) or slugify(raw_cat_id)
...
def gen_about(data):
    ...
    for lang in SUPPORTED_LANGS:
        ...
        page = {
            ...
            "contentHtml":     ab_lo.get("contentHtml")     or ab_gf.get("contentHtml")     or "",
