...
def gen_index(data):
    ...
    for lang in SUPPORTED_LANGS:
        ...
        for s in raw_slides:
            ...
            if bg or s.get("headline"):
                slides.append({
                    ...
                    "btnText":    s.get("buttonText")   or s.get("btnText")    or "",
                    "btnSubtext": s.get("buttonSubtext") or s.get("btnSubtext") or "",
                    "btnUrl":     s.get("buttonLink")   or s.get("btnUrl")    or "",
        ...
        # ── Partners strip (logos — global, label — per-lang) ────────
        raw_partners = home_gf.get("partners") or home_lo.get("partners") or []
        partners = [
            {
                "name":    p.get("name", ""),
            ...
            for p in raw_partners if p.get("logoUrl")
        ]
        partners_label = home_lo.get("partnersLabel") or home_gf.get("partnersLabel") or ""
        partners_section = {"label": partners_label} if (partners or partners_label) else None

        # ── Promo banner ─────────────────────────────────────────────
        raw_pb = home_lo.get("promoBanner") or home_gf.get("promoBanner") or {}
        promo_banner = {
        ...
        } if raw_pb.get("headline") else None
...
def gen_404(data):
    ...
    for lang in SUPPORTED_LANGS:
        ...
        if lang == "en":
            # firebase.json rewrites 404 to /404/index.html (cleanUrls: true)
            write(f"{OUTPUT_DIR}/404/index.html", html)
