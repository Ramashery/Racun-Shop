...
def gen_blog(data):
    ...
    # Страница «blog» хранится в pages/blog (document id = "blog")
    ...
    for lang in SUPPORTED_LANGS:
        ...
        page = {
            ...
            # Hero — берём из globalFields документа blog (как в данных: translations[lang])
        ...
        # Посты для данного языка
        posts = [extract(p, lang) for p in published]

        # Featured = первый пост (можно расширить логику: флаг featured в globalFields)
        featured = None
        grid_posts = posts
        for p in posts:
