...
def is_embed_code(value):
    """
    Поле "Mobile Background Image URL" в админке иногда используют не по
    прямому назначению — туда вставляют целиком HTML/JS-код анимации
    (например, вращающуюся 3D-монету), а не ссылку на картинку.
    Отличаем код от обычного URL по характерным признакам разметки.
    """
    if not value:
        return False
    v = value.strip().lower()
...
def gen_index(data):
    ...
    for lang in SUPPORTED_LANGS:
        ...
        for s in raw_slides:
            ...
            bg_mobile_is_code = is_embed_code(bg_mobile_raw)
            btn_text = s.get("buttonText") or s.get("btnText") or ""
            btn_url  = s.get("buttonLink")  or s.get("btnUrl")  or ""
            if bg or s.get("headline"):
                slides.append({
                    "bgImage":       bg,
                    # Обычная картинка — как и раньше. Если же в поле лежит
                    # HTML/JS (3D-анимация монеты), она уходит в mobileBgCode
                    # и рендерится в шаблоне через <iframe srcdoc>, а не как
                    # CSS background-image.
                    "mobileBgImage": "" if bg_mobile_is_code else bg_mobile_raw,
                    "mobileBgCode":  bg_mobile_raw if bg_mobile_is_code else "",
                    "mobileBgIsCode": bg_mobile_is_code,
