...
COPY_IGNORE = {
    ...
    # llms.txt теперь генерируется самим скриптом (gen_llms) — если в
    # репозитории случайно останется старый статический файл с этим именем,
    # он не должен затирать сгенерированную версию. robots.txt, наоборот, —
    # обычный статический файл, редактируется вручную и просто копируется.
    "llms.txt",
...
def is_embed_url(value):
    """
    Поле "Mobile Background Image URL" помимо обычной картинки может
    указывать на отдельный HTML-файл с анимацией (например, 3D-монету) —
    это ссылка вида '/assets/coin/raccoon-coin.html', а не встроенный код.
    Вставлять сам код целиком в это поле нельзя: текстовые поля в админке/БД
    обрезают значение примерно на ~124 КБ, а такие анимации обычно больше —
    поэтому анимация хранится отдельным статическим файлом и сюда идёт
    только короткая ссылка на него.
    """
    if not value:
        return False
    v = value.strip().split("?", 1)[0].split("#", 1)[0].lower()
    return v.endswith(".html") or v.endswith(".htm")
...
# слэши внутри пути (/en///products///x), произвольный регистр в пути
# (/EN/Products/X) или конечный слэш на несуществующем ровно поэтому пути
# (/en/catalog/ — страницы сохранены плоскими файлами вида catalog.html,
# а не catalog/index.html, так что GitHub Pages честно отдаёт 404, а не
# редиректит) — исправляем на клиенте: скрипт приводит путь к канонической
# форме (без дублей слэшей, в нижнем регистре, без конечного слэша) и
# сразу делает location.replace() ещё до того, как начнут грузиться
# остальные ресурсы страницы. Единая точка правды: используется и на
# обычных страницах (write()), и на 404 (она тоже проходит через write()).
# Не настоящий HTTP 301, но при отсутствии серверного контроля это лучшее,
...
# индексации.
# Query-строка целиком отбрасывается: ни один шаблон/скрипт на сайте её
# не читает (location.search нигде не используется), а её наличие —
# источник дублей контента (метки мессенджеров/соцсетей вида ?jredk,
# случайные utm и т.п.). Единственное формальное упоминание query —
# ?q= в schema.org SearchAction (см. build_home_schema) — Google обращается
# к нему напрямую, а не через переход пользователя, так что обрезка тут
# ничего не ломает. Hash (#...) сохраняется — используется в tpl_catalog.html
# для фильтров/якорей.
_YM_SCRIPT = """  <!-- Yandex.Metrika counter -->
  <script type="text/javascript">
      ...
      })(window, document,'script','https://mc.yandex.ru/metrika/tag.js?id=110636147', 'ym');

      ym(110636147, 'init', {ssr:true, webvisor:true, clickmap:true, ecommerce:"dataLayer", referrer: document.referrer, url: location.href, accurateTrackBounce:true, trackLinks:true});
  </script>
  <noscript><div><img src="https://mc.yandex.ru/watch/110636147" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
  <!-- /Yandex.Metrika counter -->
"""


_GA_SCRIPT = """  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-Q5RR21QY7K"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());

    gtag('config', 'G-Q5RR21QY7K');
  </script>
"""


_FAVICON_LINKS = """  <link rel="icon" type="image/png" href="/favicon-96x96.png" sizes="96x96" />
...
_URL_NORMALIZE_SCRIPT = """  <script>
  (function () {
    ...
    if (clean.length > 1 && clean.slice(-1) === "/") {
      clean = clean.slice(0, -1);
    }
    if (clean !== p || location.search) {
      location.replace(clean + location.hash);
    }
  })();
  </script>
"""
...
def write(path, content):
    if path.endswith(".html") and "<head>" in content:
        content = content.replace("<head>", "<head>\n" + _YM_SCRIPT + _GA_SCRIPT + _FAVICON_LINKS + _URL_NORMALIZE_SCRIPT, 1)
...
def gen_index(data):
    ...
    for lang in SUPPORTED_LANGS:
        ...
        for s in raw_slides:
            ...
            bg_mobile_raw = s.get("mobileBackgroundImageUrl") or s.get("mobileBgImage") or ""
            bg_mobile_is_embed = is_embed_url(bg_mobile_raw)
            btn_text = s.get("buttonText") or s.get("btnText") or ""
            btn_url  = s.get("buttonLink")  or s.get("btnUrl")  or ""
            if bg or s.get("headline"):
                slides.append({
                    "bgImage":       bg,
                    # Обычная картинка — как и раньше. Если поле указывает на
                    # .html-файл (анимация монеты и т.п.), она рендерится в
                    # шаблоне через <iframe src="...">, а не как CSS
                    # background-image.
                    "mobileBgImage": "" if bg_mobile_is_embed else bg_mobile_raw,
                    "mobileBgUrl":   bg_mobile_raw if bg_mobile_is_embed else "",
                    "mobileBgIsEmbed": bg_mobile_is_embed,
