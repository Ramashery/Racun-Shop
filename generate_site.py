...
def gen_categories(data):
    """
    Отдельные SEO-страницы для категорий: /{lang}/catalog/{slug}.
    Используют translations.{lang}.{name, description, seoTitle, seoDescription,
    metaDescription} и корневые поля color/id/slug/status категории (без globalFields —
    ...
    for lang in SUPPORTED_LANGS:
        ...
        for c in active_categories:
            ...
            cats_for_lang.append({
                "id":    c_id,
                "slug":  slugify(c.get("slug") or c_id),
        ...
        for c_raw in active_categories:
            ...
            if not c_id:
                continue
            c_lo      = loc(c_raw, lang)
            cat_slug  = slugify(c_raw.get("slug") or c_id)
            ...
            page = {
                ...
                "ogDescription":   c_lo.get("seoDescription") or c_lo.get("metaDescription") or "",
            ...
            write(f"{OUTPUT_DIR}/{lang}/catalog/{cat_slug}.html", html)
...
def _entry_url(entry_type, base_url, lang, slug, cat):
    ...
    if entry_type == "category_index":
        return f"{base_url}/{lang}/catalog/{cat}"
    # post
