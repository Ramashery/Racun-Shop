...
def _parse_date(value):
    ...
    Пытаемся привести к ISO-дате для datePublished; если не получилось —
    возвращаем '' и просто не добавляем поле (лучше отсутствие, чем мусор)."""
    if not value:
        return ""
...
def build_post_schema(lang, post):
    ...
    if author_name:
        ...
        node["author"] = {"@id": f"{BASE_URL}/#organization"}
