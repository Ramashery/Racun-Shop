...
# firebase.json/_redirects/.htaccess), поэтому URL вида /en///products///x
# (который GitHub Pages отдаёт как 200, просто нормализуя путь при поиске
# файла) исправляем на клиенте: скрипт схлопывает лишние слэши и сразу же
# делает location.replace() на канонический путь — ещё до того, как
# начнут грузиться остальные ресурсы страницы. Не настоящий HTTP 301, но
# при отсутствии серверного контроля это лучшее, что доступно "из коробки"
# без внешних сервисов (Cloudflare и т.п.) — см. также rel="canonical" на
# каждой странице как дополнительная подстраховка для индексации.
_SLASH_COLLAPSE_SCRIPT = """  <script>
  (function () {
    var p = location.pathname;
    if (/\\/{2,}/.test(p)) {
      location.replace(p.replace(/\\/{2,}/g, "/") + location.search + location.hash);
    }
  })();
  </script>
"""
