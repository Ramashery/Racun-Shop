...
_BREADCRUMB_LABELS = {"home": "Home", "catalog": "Catalog"}
...
def build_product_schema(lang, product):
    ...
    if dosage:
        dosage_val = ", ".join(str(d) for d in dosage) if isinstance(dosage, list) else str(dosage)
        if dosage_val:
    ...
    if ingredients:
        ing_val = ", ".join(str(i) for i in ingredients) if isinstance(ingredients, list) else str(ingredients)
        if ing_val:
