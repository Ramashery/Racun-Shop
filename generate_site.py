...
def build_home_schema(lang, page, contacts, reviews, products):
    ...
    # Показываем весь ассортимент, но помечаем "@type": "Product" (+ offers)
    # только у товаров с ценой в базе — Google требует offers/review/
    # aggregateRating у любого узла типа Product. Товары без цены (RAKUN —
    # опт, цена часто "по запросу") идут нейтральным типом Thing: это всё
    # ещё валидная ссылка с картинкой и названием, но без коммерческой
    # семантики Product, поэтому Google не проверяет её по правилам Offer
    # и не даёт ложных ошибок.
    listed = [p for p in products if p.get("name")][:12]
    if listed:
        item_list = {
            "@type": "ItemList",
            "@id": f"{page_url}#featured-products",
            "name": "RAKUN",
            "itemListElement": [],
        }
        for i, p in enumerate(listed, start=1):
            ...
            item_list["itemListElement"].append({"@type": "ListItem", "position": i, "item": node})
